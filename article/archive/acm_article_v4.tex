...
denote the directed configuration graph introduced in Section~\ref{sec:model},
where each vertex
...
\begin{itemize}
    \item $L_v$ denotes the lifecycle state,
    \item $Q_v$ denotes the governance quality state,
    \item $A_v$ denotes the assurance state,
    \item $I_v$ denotes the impact state,
    \item $E_v$ denotes the execution eligibility.
\end{itemize}

Some components of $\Gamma$ are directly assigned by governance processes,
...
Each ACI revision is associated with a lifecycle mapping
...
Lifecycle transitions are defined by the transition function
...
Whenever such a transition is requested, ACM shall reject the operation and
require the creation of a new revision identifier.
...
of concurrent governance operations.
...
\begin{table}[t]
\caption{Normative lifecycle transition matrix.}
...
single monolithic automaton whose state space would grow exponentially.
...
introduced later in Section~\ref{subsec:impact}.
...
\centering
Placeholder for TikZ governance state machine diagram.
}}
\caption{Orthogonal governance state machines composing the ACM governance semantics.}
...
The previous subsection introduced impact as one of the orthogonal governance
dimensions composing the ACM semantic model. This subsection formalizes the
rules governing how local configuration changes propagate throughout an
agentic configuration graph. The objective is not to determine whether a
configuration remains correct, but rather to identify every configuration item
whose governance status may have been affected by an upstream modification.

Unlike conventional software dependency analysis, ACM considers governance
dependencies rather than execution dependencies alone. A modification may
invalidate validation evidence, governance approvals, deployment decisions,
runtime assumptions, or configuration consistency even when execution remains
possible. Consequently, impact propagation constitutes a governance analysis
performed prior to eligibility evaluation.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Propagation Operator}
...
Since the governance graph is finite,

\[
|V|<\infty,
\]

the propagation algorithm always converges.
...
While impact propagation identifies the subset of configuration items whose
governance status must be reconsidered, deployment decisions require an
additional reasoning step. ACM therefore introduces an explicit eligibility
semantics responsible for determining whether a given ACI revision may execute
under the current governance context.

Eligibility does not constitute an intrinsic property of an ACI.
Instead, it is derived from the evaluation of multiple governance dimensions,
thereby separating governance observations from governance decisions.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Normative Evaluation Rules}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Eligibility Ordering}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Eligibility Composition}
...
Assurance constitutes the evidential dimension of governance. Whereas
\emph{quality} expresses the outcome of evaluations (e.g., whether a component
is acceptable or not), assurance characterizes the degree of confidence that
this judgment is supported by an appropriate and reproducible body of evidence.
This distinction follows a long-standing principle inherited from Software
Configuration Management and software assurance practices: a component may be
considered satisfactory while still lacking sufficient objective evidence, and
conversely, a fully assessed component may legitimately fail its evaluation.
...
Assurance is intentionally orthogonal to both lifecycle maturity and quality.
Promotion through the lifecycle is a governance decision, while quality
represents the result of evaluations. Assurance measures neither of these
dimensions directly; instead, it evaluates the completeness and applicability
...
evidence for an entire composite configuration.
...
Equation~(\ref{eq:coverage}) measures only the completeness of available
...
Only evidence satisfying Equation~(\ref{eq:applicable}) contributes to the
coverage defined in Equation~(\ref{eq:coverage}).
...
deterministic order.}
\label{fig:governance_state_propagation}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\emph{configuration state} and \emph{execution state}. Configuration revisions
represent immutable governance decisions, whereas runtime events capture the
observable behaviour of a particular execution instance. Runtime information
therefore augments the governance model but never alters the published
...
of execution provenance, runtime topology, governance relationships, and
...
Many contemporary agentic frameworks allow agents to be created dynamically
during execution. Such runtime entities may result from planning algorithms,
delegation mechanisms, agent factories, or adaptive workflow expansion. While
this capability significantly increases execution flexibility, it also raises
important governance challenges. Dynamically created agents may execute prompts,
invoke tools, or access resources that were not explicitly represented in the
published configuration baseline.

Rather than treating dynamically created agents as ordinary configuration
revisions, ACM distinguishes between the immutable specification of an agent
and its runtime instantiations. This separation preserves the immutability of
published baselines while enabling complete traceability of dynamically
generated execution structures.
...
Each override is evaluated against the governance policy attached to the
...
Overrides therefore modify only runtime behaviour without altering the
...
traceability, explainability and reproducibility.
...
can be reproduced independently of the implementation.
...
The results presented in the following subsection therefore report
Configuration Coverage (\(C_f\)), Semantic Preservation (\(S_f\)), and
Information Loss (\(I_f\)) separately for each evaluated framework, providing a
transparent and reproducible assessment of the expressiveness of the ACM
reference model.
...
The successful execution of the complete scenario suite demonstrates that the
ACM reference model is sufficiently expressive to represent the governance
constructs required by the evaluated agentic systems. The semantic projection
process preserved the configuration concepts necessary for lifecycle
management, provenance tracking, dependency representation, runtime
traceability, and governance evaluation. Unsupported framework-specific
constructs were explicitly identified through the projection reports rather
than silently discarded, allowing the limits of the current reference model to
remain observable.

\paragraph{Contribution to RQ2 (Semantic Preservation).}

The operational scenarios confirm that governance semantics remain consistent
after projection into ACM. Lifecycle transitions, dependency propagation,
assurance evaluation, eligibility computation, runtime traceability, and drift
detection behaved according to the formal semantics defined in
Section~\ref{sec:governance_semantics}. The deterministic replay experiments
further demonstrated that governance states can be reconstructed
reproducibly from recorded runtime events independently of probabilistic LLM
outputs.

Two scenarios (ACM-S02 and ACM-S20) initially revealed discrepancies between
the specification and the reference implementation. Rather than weakening the
evaluation, these discrepancies strengthened the reference model by leading to
normative refinements of the governance kernel before completion of the
experimental campaign. Their resolution illustrates the value of the
scenario-based validation methodology adopted in this work.

\paragraph{Contribution to RQ3 (Framework Independence).}

The evaluation demonstrated that equivalent configurations originating from
LangGraph and CrewAI can be projected into equivalent ACM representations for
the governance constructs covered by this study. Runtime normalization and
projection consistency remained deterministic across the evaluated scenarios,
supporting the framework-independent design objective of ACM.

These results should nevertheless be interpreted within the experimental scope
of the present work. They provide evidence that the proposed governance model
can normalize representative configurations from two distinct orchestration
paradigms, but they do not constitute a claim of universal compatibility with
arbitrary agentic frameworks.
...
Finally, the evaluation highlights the current scope of ACM. The experiments
...
The objective of ACM is to provide a reference model for the governance of
agentic configurations rather than a complete execution platform for autonomous
AI systems. Consequently, the current version intentionally focuses on
configuration governance, lifecycle management, provenance, assurance, and
runtime traceability, while leaving several higher-level reasoning capabilities
outside the scope of the reference model.

Table~\ref{tab:limitations_scope} summarizes the current coverage of ACM and
the principal capabilities intentionally deferred to future work.
...
These limitations originate from deliberate design choices rather than
technical constraints. ACM was intentionally designed to establish a stable
governance layer before addressing higher-level cognitive capabilities. Mixing
governance semantics with planning, learning, or autonomous reasoning would
have significantly increased the complexity of the reference model while making
its formal semantics considerably more difficult to specify and validate.

The current reference model therefore assumes that planning algorithms,
reasoning engines, learning mechanisms, and execution strategies operate as
external components whose observable configuration and runtime behavior can be
governed through ACM. In other words, ACM governs \emph{how} these components
are configured, versioned, validated, deployed, and traced, but not
\emph{how} they internally produce decisions.

This separation has two important implications. First, the governance model
remains independent of the evolution of underlying AI techniques, allowing new
reasoning paradigms or orchestration frameworks to be incorporated through
semantic projection rather than modifications of the governance kernel.
Second, the current evaluation should not be interpreted as validating the
correctness or performance of autonomous reasoning algorithms themselves, but
rather the ability of ACM to govern their configuration throughout the system
lifecycle.

Finally, the experimental validation presented in this work remains limited to
the representative orchestration frameworks and configuration scales evaluated
by the reference implementation. Although the results provide encouraging
evidence regarding the generality of the proposed governance semantics,
additional empirical validation across larger industrial deployments and
broader classes of agentic systems remains necessary before broader
...
Finally, the publication of the reference implementation, the declarative test
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\appendices
\section{Implemented Evaluation Scenarios}
...
the behaviour of the reference implementation. :contentReference[oaicite:0]{index=0}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Configuration and Baseline Integrity}
...
complete configuration graphs. :contentReference[oaicite:2]{index=2}
...
revision identities was observed. :contentReference[oaicite:3]{index=3}
...
baseline.
Immutability was preserved throughout the experiment. :contentReference[oaicite:4]{index=4}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Propagation and Assurance}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Runtime, Replay and Drift}
