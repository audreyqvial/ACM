...
\begin{table*}[t]
	...
	\begin{tabular}{lccc}
		...
		Declared by adapter & 4 & 2 (Flow only), 3 (Crew only), 5 (Flow + Crew) & 1 (Agent only), 5 (Agent + Graph) \\
		\bottomrule
	\end{tabular}
\end{table*}
...
\paragraph{Agentic typing of configuration items}. Removing the agentic specialization of the ACI taxonomy reduces the model to conventional software configuration management operating over opaque artifacts. The immediate consequence is the loss of relationship typing: without distinguishing agents, prompts, tools, models, and policies, the relationship types that carry propagation semantics (Section ~\ref{sec:propagation_policies}) can no longer be assigned, and impact propagation degenerates into undifferentiated reachability. The typing is therefore not merely descriptive convenience: within ACM, it provides the semantic basis required to associate heterogeneous configuration relationships with governance policies and impact semantics.
...
(\texttt{langgraph}, \texttt{crewai}, \texttt{openai\_agents}). Two
methodological caveats are retained here exactly as emitted by the
tooling, since they bound the strength of the reported evidence: the
oracle digest verification was not activated in the reported impact run
(\texttt{digest\_verified: false}), and three scenarios pass with a
documented, deliberate deviation from the original plan
(\texttt{pass\_with\_deviation}). Both are discussed in
...
campaign, $295$ tests passed with no failures and no skipped tests;
...
\begin{table}[htbp]
	...
	\caption{Propagation-fixture metrics (harness). Iterations denote
		fixed-point iterations; convergence was reached in every case.}
...
explicitly and extracts it, signalling opaque conditions. The CrewAI
Flow topology is not statically introspectable in recent versions
(edges are resolved at execution time) and is therefore partly supplied
by adapter metadata; the state schema is reported \texttt{unsupported}
rather than silently dropped. The OpenAI Agents SDK exposes the handoff
delegation topology directly on the objects, so edges are extracted
without adapter-declared topology, while the mapping of nodes to ACM
references (agent/prompt/model/tool) remains adapter-declared, exactly
as for LangGraph. This documented-loss behavior is the property that
distinguishes semantic projection from ad hoc per-framework extraction.
...
throughout.
Each case records a SHA-256 digest of its frozen oracle; in this run, however, the digest was computed but not checked against a fixed reference (\texttt{digest-verified:false}), so oracle immutability is asserted by construction of the harness rather than cryptographically verified end-to-end.
